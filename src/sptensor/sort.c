...
void spt_SwapValues(sptSparseTensor *tsr, sptNnzIndex ind1, sptNnzIndex ind2) {
    ...
    int index = tsr->stidx[ind1];
    tsr->stidx[ind1] = tsr->stidx[ind2];
    tsr->stidx[ind2] = index;
}
 ...
 * @return -1 for less, 0 for equal, 1 for greater
 */
int spt_SparseTensorHiCOOCompareIndicesCustomize(sptSparseTensorHiCOO * const tsr1, sptNnzIndex loc1, sptIndex * const mode_order_1, sptSparseTensorHiCOO * const tsr2, sptNnzIndex loc2, sptIndex * const mode_order_2, sptIndex num_ncmodes)
{
    sptIndex i, m1, m2;
    sptIndex eleind1, eleind2;
    for(i = 0; i < num_ncmodes; ++ i) {
        m1 = mode_order_1[i];
        m2 = mode_order_2[i];
        eleind1 = tsr1->binds[m1].data[loc1];
        eleind2 = tsr2->binds[m2].data[loc2];
        if(eleind1 < eleind2) {
            return -1;
        } else if(eleind1 > eleind2) {
            return 1;
...
unsigned int MultShift_htHashCode(unsigned long long key, unsigned int shift){
    unsigned long long res = key*14695981039346656037;
    return res >> shift; // 14695981039346656037 from FNV-1a hash function between(2^63, 2^64-1)
}
unsigned int MultShift_htHashCode1(unsigned long long key, unsigned int shift){
    unsigned long long res = key*11400714819323198485; // From DJB2 HASH
    // unsigned long long res = key*144066263297769815596495629667062367629; // Another from FNV
    return res >> shift; // 14695981039346656037 from FNV-1a hash function between(2^63, 2^64-1)
}
...
// Hash table for SPA: LinearProbing
LP_table_t *LP_htCreate(const unsigned int ht_size){
    LP_table_t *LP_t = (LP_table_t*)malloc(sizeof(LP_table_t));
    LP_t->size = (1 << ht_size);
    LP_t->shift = 64 - ht_size;
    LP_t->len = 0;
    // LP_t->item = (LP_node_t*)malloc((1 << ht_size)*sizeof(LP_node_t));
    LP_t->stat = (char*)malloc((1 << ht_size)*sizeof(int));
    memset(LP_t->stat, 0, (1 << ht_size)*sizeof(char));
    // LP_t->stat = (int*)calloc((1 << ht_size),sizeof(int));
    LP_t->key = (unsigned long long*)malloc((1 << ht_size)*sizeof(unsigned long long));
    LP_t->val = (sptValue*)malloc((1 << ht_size)*sizeof(sptValue));
    // LP_t->item = (LP_node_t*)calloc((1 << ht_size),sizeof(LP_node_t));
    // memset(LP_t->item, 0, sizeof(LP_node_t)*LP_t->size);
    return LP_t;
}

void LP_htUpdate(LP_table_t *LP_t, unsigned long long key, sptValue val){
    unsigned int pos = MultShift_htHashCode(key, LP_t->shift);
    int size = LP_t->size;
    while(size--){
        // if(LP_t->item[pos].key == key && LP_t->item[pos].stat == 1){
        if(LP_t->key[pos] == key && LP_t->stat[pos] == 1){
            // LP_t->item[pos].val = val;
            LP_t->val[pos] = val;
            return;
        }
        else{
            pos++;
            if(pos >= LP_t->size) pos = 0;
        }
    }
}
unsigned int LP_htInsert(LP_table_t *LP_t, unsigned long long key, sptValue val){
    if(LP_t == NULL){
        printf("Invalid LP hash table!\n");
        return;
    }
    if(LP_t->len >= LP_t->size){
        printf("No empty slot in LP hash table!\n");
        // return; 
        exit(0);
    }
    unsigned int pos = MultShift_htHashCode(key, LP_t->shift);
    int size = LP_t->size;
    while(size--){
        // if(LP_t->item[pos].stat == 0){ //empty
        //     LP_t->item[pos].stat = 1;
        //     LP_t->item[pos].key = key;
        //     LP_t->item[pos].val = val; 
        //     LP_t->len ++;
        //     return;
        // }
        if(LP_t->stat[pos] == 0){ //empty
            LP_t->stat[pos] = 1;
            LP_t->key[pos] = key;
            LP_t->val[pos] = val; 
            LP_t->len ++;
        ...
        // else if(LP_t->item[pos].stat == 1){
        else if(LP_t->stat[pos] == 1){
            pos++;
            if(pos >= LP_t->size) pos = 0;
        }
...
sptValue LP_htGet(LP_table_t *LP_t, unsigned long long key){
    if(LP_t->len == 0) return LONG_MIN;
    unsigned int pos = MultShift_htHashCode(key, LP_t->shift);
    int size = LP_t->size;
    while(size--){
        // if(LP_t->item[pos].key == key && LP_t->item[pos].stat == 1){
        if(LP_t->key[pos] == key && LP_t->stat[pos] == 1){
            // return LP_t->item[pos].val;
            return LP_t->val[pos];
        }
        // else if(LP_t->item[pos].stat == 0){
        else if(LP_t->stat[pos] == 0){
            return LONG_MIN;
        }
        else{
            pos++;
...
// Hash table for SPA: Quadratic Probing
...
// Hash table for SPA: Hopscotch hash
...
unsigned int HS_htInsert(LP_table_t *LP_t, unsigned long long key, sptValue val){
    ...
    printf("Hopscotch Hashing failed\n");
    exit(-1);
    return -1;
}
...
// Hash table for SPA: Perfect hash
void PHF_htUpdate(LP_table_t *LP_t, unsigned long long key, sptValue val, int min_key, int sub_ht_size, int key_split_size){
    int size = LP_t->size;
    unsigned int pos1 = (key - min_key) & 0b11; // the sub table id
    unsigned int init_shift = pos1*sub_ht_size;
    unsigned int init_loc = key - pos1*key_split_size - min_key;
    unsigned int pos = init_shift + init_loc % 127; 
    if (LP_t->key[pos] == key && LP_t->stat[pos] == 1) {
        LP_t->val[pos] = val;
        return;
    }else{ // one hop
        // pos = init_shift + init_loc %  383 + 127;
        // if(LP_t->key[pos] == key && LP_t->stat[pos] == 1){
        //     // LP_t->item[pos].val = val;
        //     LP_t->val[pos] = val;
        //     return;
    ...
    printf("PHF Hashing update failed\n");
    exit(-1);
    return -1;
}
unsigned int PHF_htInsert(LP_table_t *LP_t, unsigned long long key, sptValue val, int min_key, int sub_ht_size, int key_split_size){
    ...
    printf("PHF Hashing insert failed\n");
    exit(-1);
    return -1;
}
...
// Hash table for SPA: SEGMENT hash 1
...
//     int size = LP_t->size;
//     unsigned int pos1 = key_idx >> nnz_each_seg_shift; // the sub table id;
//     unsigned int init_shift = pos1*sub_ht_size;
//     unsigned long long res = key *11400714819323198485;
//     unsigned int pos = init_shift + res >> 55;  
//     // assert((pos < size) && ("Assertion failed: pos >= size"));
//     if (LP_t->key[pos] == key && LP_t->stat[pos] == 1) {
//         LP_t->val[pos] = val;
//         return;
//     }else{ // one hop
...
//     }
//     printf("SEG Hashing update failed\n");
//     exit(-1);
//     return -1;
// }
...
//     unsigned long long res = key *11400714819323198485;
//     unsigned int pos = init_shift + res >> 55; 
//     // assert((pos < size) && ("Assertion failed in the init: pos >= size"));
//     // printf("key:%llu, key_idx:%u, sub_ht_idx:%u, sub_ht_size:%d\n", key, key_idx, pos1, sub_ht_size);
//     if (LP_t->stat[pos] == 0) {
//         LP_t->stat[pos] = 1;
//         LP_t->key[pos] = key;
//         LP_t->val[pos] = val; 
...
//     }else{ // one hop
//         pos = (pos + 1)&(size-1);
//         // assert((pos < size) && ("Assertion failed in the first: pos >= size"));
//         if (LP_t->stat[pos] == 0) {
//             LP_t->stat[pos] = 1;
//             LP_t->key[pos] = key;
//             LP_t->val[pos] = val; 
...
//             unsigned long long res = key *14695981039346656037;
//             unsigned int pos  = init_shift + res >> 55; 
//             // assert((pos < size) && ("Assertion failed in the second: pos >= size"));
//             if (LP_t->stat[pos] == 0) {
//                 LP_t->stat[pos] = 1;
//                 LP_t->key[pos] = key;
//                 LP_t->val[pos] = val; 
...
//     }
//     printf("SEG Hashing insert failed\n");
//     exit(-1);
//     return -1;
// }
...
// Hash table for SPA: SEGMENT hash 2
void SEG_htUpdate(LP_table_t *LP_t, unsigned long long key, sptValue val, unsigned int key_idx, int nnz_each_seg_shift, int sub_ht_size){
    int size = LP_t->size;
    unsigned int pos1 = key_idx >> nnz_each_seg_shift; // the sub table id;
    unsigned int init_shift = pos1*sub_ht_size;
    unsigned int pos = init_shift + (key_idx&511);  
    // assert((pos < size) && ("Assertion failed: pos >= size"));
    if (LP_t->key[pos] == key && LP_t->stat[pos] == 1) {
        LP_t->val[pos] = val;
        return;
...
unsigned int SEG_htInsert(LP_table_t *LP_t, unsigned long long key, sptValue val, unsigned int key_idx, int nnz_each_seg_shift, int sub_ht_size){
    ...
    unsigned int pos = init_shift + (key_idx&511); 
    // assert((pos < size) && ("Assertion failed in the init: pos >= size"));
    // printf("key:%llu, key_idx:%u, nnz_seg_shift:%d, sub_ht_idx:%u, sub_ht_size:%d ", key, key_idx,nnz_each_seg_shift, pos1, sub_ht_size);
    // printf("pos:%u, stat:%d, key_now:%u\n", pos, LP_t->stat[pos], LP_t->key[pos]);
    if (LP_t->stat[pos] == 0) {
        LP_t->stat[pos] = 1;
        LP_t->key[pos] = key;
        LP_t->val[pos] = val; 
...
// Hash table for SPA: Cuckcoo hashing
CK_table_t *CK_htCreate(const unsigned int ht_size){
    CK_table_t *CK_t = (CK_table_t*)malloc(sizeof(CK_table_t));
    CK_t->size = (1 << ht_size);
    CK_t->shift = 64 - ht_size + 1;
    CK_t->len = 0;
    unsigned int tmp_sz = CK_t->size >> 1;
    // LP_t->item = (LP_node_t*)malloc((1 << ht_size)*sizeof(LP_node_t));
    CK_t->stat[0] = (char*)malloc(tmp_sz*sizeof(char));
    CK_t->stat[1] = (char*)malloc(tmp_sz*sizeof(char));
    memset(CK_t->stat[0], 0, tmp_sz*sizeof(char));
    memset(CK_t->stat[1], 0, tmp_sz*sizeof(char));
    CK_t->key[0] = (unsigned long long*)malloc(tmp_sz*sizeof(unsigned long long));
    CK_t->key[1] = (unsigned long long*)malloc(tmp_sz*sizeof(unsigned long long));
    // memset(CK_t->key[0], 0xff, tmp_sz*sizeof(unsigned long long));
    // memset(CK_t->key[1], 0xff, tmp_sz*sizeof(unsigned long long));

    CK_t->val[0] = (sptValue*)malloc(tmp_sz*sizeof(sptValue));
    CK_t->val[1] = (sptValue*)malloc(tmp_sz*sizeof(sptValue));
...
unsigned int CK_htHashCode(int func, unsigned long long key, unsigned int shift){
    switch(func){
        case 0: return MultShift_htHashCode(key, shift);
        // case 0: return Murmur3_32_htHashCode(&key, 8, 40, shift-32);
        case 1: return MultShift_htHashCode1(key, shift);
        // case 1: return Murmur3_32_htHashCode(&key, 8, 44, shift-32);
    }
}

unsigned long long CK_htGet(CK_table_t *CK_t, unsigned long long key){
    if(CK_t->len == 0) return ULLONG_MAX;
    unsigned int pos0 = CK_htHashCode(0, key, CK_t->shift);
    // if(CK_t->key[0][pos0] != ULLONG_MAX && CK_t->key[0][pos0] == key)
    if(CK_t->stat[0][pos0] == 1 && CK_t->key[0][pos0] == key)
        return pos0;
    unsigned int pos1 = CK_htHashCode(1, key, CK_t->shift);
    if(CK_t->stat[1][pos1] == 1 && CK_t->key[1][pos1] == key)
        return pos1;
    return ULLONG_MAX;
}
// extern int Count_Loop;
// unsigned int CK_htInsert(CK_table_t *CK_t, unsigned int tableidx, unsigned long long key, sptValue val){
//     unsigned int pos = CK_htHashCode(tableidx, key, CK_t->shift);
//     // Count_Loop++;
//     // if(Count_Loop > 500) {
//     //     printf("Reach MaxLoop!!!!!!!!!!!!!!!!!!!!! with load factor: %.6lf\n", 1.0*CK_t->len/CK_t->size);
//     //     exit(-1);
//     // }
//     if(CK_t->stat[tableidx][pos] == 0){
//         CK_t->stat[tableidx][pos] = 1;
//         CK_t->key[tableidx][pos] = key;
//         CK_t->val[tableidx][pos] = val;
//         CK_t->len ++;
//         if(tableidx == 0) return pos;
//         else return (pos | 0x80000000); // highest bit represent the tableidx
//     }else{
//         unsigned long long tmp_key = CK_t->key[tableidx][pos];
//         sptValue tmp_val = CK_t->val[tableidx][pos];
//         CK_t->key[tableidx][pos] = key;
//         CK_t->val[tableidx][pos] = val;
//         // CK_t->len ++;
//         unsigned int new_pos = CK_htInsert(CK_t, (tableidx+1)%2, tmp_key, tmp_val);
        
//         if(tableidx == 0) return new_pos;
//         else return (new_pos | 0x80000000); // highest bit represent the tableidx
//     }
// }
...
unsigned int CK_htInsert(CK_table_t *CK_t, unsigned int tableidx, unsigned long long key, sptValue val){
    ...
    // while(1){
        if(CK_t->stat[tableidx][pos] == 0){
            ...
            CK_t->val[tableidx][pos] = val;
            // CK_t->len ++;
            tableidx = (tableidx+1)%2;
            key = tmp_key;
            val = tmp_val;
            pos = CK_htHashCode(tableidx, key, CK_t->shift);
        }    
    }
    printf("CK_htInsert reached the MaxLoop!!!!!!!!!!! Loading factor: %.6f\n", 1.0*CK_t->len/CK_t->size);
    exit(1);
}
// unsigned int CK_htInsert(CK_table_t *CK_t, unsigned int tableidx, unsigned long long key, sptValue val){
//     // unsigned int pos = CK_htHashCode(tableidx, key, CK_t->shift);
...
//     // }
//     int Count_Loop = 0;
//     unsigned long long tmp_key = key;
//     sptValue tmp_val = val;
//     while(Count_Loop++ < 5000){
//         unsigned int pos0 = CK_htHashCode(0, tmp_key, CK_t->shift);
//         if(CK_t->stat[0][pos0] == 0){
//             CK_t->stat[0][pos0] = 1;
//             CK_t->key[0][pos0] = tmp_key;
//             CK_t->val[0][pos0] = tmp_val;
//             CK_t->len ++;
//             return pos0;
//         }else{ // check the second table
//             unsigned int pos1 = CK_htHashCode(1, tmp_key, CK_t->shift);
//             if(CK_t->stat[1][pos1] == 0){ // Success insert in the second table
//                 CK_t->stat[1][pos1] = 1;
//                 CK_t->key[1][pos1] = key;
//                 CK_t->val[1][pos1] = val;
//                 CK_t->len ++;
//                 return (pos1 | 0x80000000);
//             }
//             else{ // need to kick out the old element
//                 unsigned long long old_key = CK_t->key[1][pos1];
//                 sptValue old_val = CK_t->val[1][pos1];
//                 CK_t->key[1][pos1] = tmp_key;
//                 CK_t->val[1][pos1] = tmp_val;
//                 tmp_key = old_key;
//                 tmp_val = old_val;  
//             }
//         }
...
void CK_htUpdate(CK_table_t *CK_t, unsigned int pos, unsigned long long key, sptValue val){
    unsigned int tableidx = (0x80000000 & pos) >> 31;
    unsigned int real_pos = pos & 0x7fffffff;
    // sptValue old_value = CK_t->val[tableidx][real_pos];
    CK_t->val[tableidx][real_pos] += val;
}
...
table_t *htCreate(const unsigned int size){
    ...
    t->shift = 64 - size;
    // ht_size = size;
    t->list = ( node_t**)malloc(sizeof( node_t*)*t->size);
    unsigned int i;
    for(i=0;i<t->size;i++)
        t->list[i] = NULL;
    return t;
...
// unsigned int ht_shift = 48;
// unsigned int htHashCode_size(unsigned long long key, unsigned int ht_size){
unsigned int htHashCode_size(unsigned long long key, unsigned int shift){
    // return key % ht_size;
    // return key & (ht_size - 1);
    unsigned long long res = key*14695981039346656037;
    return res >> shift;
...
void htUpdate( table_t *t, unsigned long long key, sptValue val){
    unsigned int pos = htHashCode_size(key, t->shift);
...
unsigned int htInsert( table_t *t, unsigned long long key, sptValue val){
    ...
    node_t *list = t->list[pos];
    if(list == NULL) result = pos;
    else result = UINT_MAX;
     // node_t *temp = list;
    newNode->key = key;
    newNode->val = val;
    newNode->next = list;
...
tensor_table_t *tensor_htCreate(const unsigned int size){
    ...
    t->size = 1 << size;
    t->shift = 64 - size;
    t->list = ( tensor_node_t**) malloc(sizeof( tensor_node_t*)*t->size);
    unsigned int i;
    for(i=0;i<t->size;i++)
        t->list[i] = NULL;
    return t;
...
void tensor_htUpdate(tensor_table_t *t, unsigned long long key_cmode,  unsigned long long key_fmode, sptValue value){
    // unsigned int pos = tensor_htHashCode_size(key_cmode, t->size);
    unsigned int pos = MultShift_htHashCode(key_cmode, t->shift);
...
void tensor_htInsert(tensor_table_t *t, unsigned long long key_cmodes, unsigned long long key_fmodes, sptValue value){
    ...
    newNode->val = my_vec;
    // unsigned int pos = tensor_htHashCode_size(key_cmodes, t->size);
    unsigned int pos = MultShift_htHashCode(key_cmodes, t->shift);
    tensor_node_t *list = t->list[pos];
    newNode->next = list;
    t->list[pos] = newNode;
}
...
tensor_value tensor_htGet( tensor_table_t *t, unsigned long long key){
    // unsigned int pos = tensor_htHashCode_size(key, t->size);
    unsigned int pos = MultShift_htHashCode(key, t->shift);
    tensor_node_t *list = t->list[pos];
    tensor_node_t *temp = list;
...
LP_tensor_table_t* LP_tensor_htCreate(unsigned int ht_size){
    LP_tensor_table_t* LP_t = (LP_tensor_table_t*)malloc(sizeof(LP_tensor_table_t));
    LP_t->size = (1 << ht_size);
    LP_t->shift = 64 - ht_size;
    LP_t->len = 0;
    LP_t->LP_node_list = (LP_tensor_node_t *)malloc(LP_t->size * sizeof(LP_tensor_node_t));
    for(sptNnzIndex i = 0; i < LP_t->size; i++)
        LP_t->LP_node_list[i].stat = 0;
    return LP_t;
}
void LP_tensor_htUpdate(LP_tensor_table_t *LP_t, unsigned long long key_cmodes,  unsigned long long key_fmodes, sptValue value){
    unsigned int pos =  MultShift_htHashCode(key_cmodes, LP_t->shift);
    int size = LP_t->size;
    while(size--){
        if(LP_t->LP_node_list[pos].key == key_cmodes && LP_t->LP_node_list[pos].stat == 1){
            tensor_htAppendValueVector(&(LP_t->LP_node_list[pos].val_ptr), key_fmodes, value); 
        }
        else{
            pos++;
            if(pos >= LP_t->size) pos = 0;
...
unsigned int LP_tensor_htInsert(LP_tensor_table_t *LP_t, unsigned long long key_cmodes, unsigned long long key_fmodes, sptValue value){
    ...
    while(size--){
        // if(LP_t->stat[pos] == 0){
        //     LP_t->stat[pos] = 1;
        //     LP_t->key[pos] = key_cmodes;
        if(LP_t->LP_node_list[pos].stat == 0){
            LP_t->LP_node_list[pos].stat = 1;
            LP_t->LP_node_list[pos].key = key_cmodes;
            tensor_htNewValueVector(&(LP_t->LP_node_list[pos].val_ptr), 0, 0);
            tensor_htAppendValueVector(&(LP_t->LP_node_list[pos].val_ptr), key_fmodes, value); 
            LP_t->len++;
            return pos;
        }
...
tensor_value LP_tensor_htGet(LP_tensor_table_t* LP_t, unsigned long long key_cmodes){
    unsigned int pos =  MultShift_htHashCode(key_cmodes, LP_t->shift);
    int size = LP_t->size;
    while(size--){
        if(LP_t->LP_node_list[pos].stat == 0){
            LP_t->LP_node_list[pos].val_ptr.len = 0;
            return LP_t->LP_node_list[pos].val_ptr;
        }
        else if(LP_t->LP_node_list[pos].key == key_cmodes && LP_t->LP_node_list[pos].stat == 1){
            return LP_t->LP_node_list[pos].val_ptr;
        }
        else{
            pos++;
...
void LP_tensor_htFree(LP_tensor_table_t* LP_t){
    free(LP_t->LP_node_list);
    free(LP_t);
}

/*CK_tensor_table*/
CK_tensor_table_t* CK_tensor_htCreate(unsigned int ht_size){
    CK_tensor_table_t* CK_t = (CK_tensor_table_t*)malloc(sizeof(CK_tensor_table_t));
    CK_t->size = (1 << ht_size);
    CK_t->shift = 64 - ht_size + 1;
    CK_t->len = 0;
    CK_t->CK_node_list[0] = (CK_tensor_node_t *)malloc(CK_t->size/2 * sizeof(CK_tensor_node_t));
    CK_t->CK_node_list[1] = (CK_tensor_node_t *)malloc(CK_t->size/2 * sizeof(CK_tensor_node_t));
    for(sptNnzIndex i = 0; i < CK_t->size/2; i++){
        CK_t->CK_node_list[0][i].stat = 0;
        CK_t->CK_node_list[1][i].stat = 0;
    }
    return CK_t;
}

tensor_value CK_tensor_htGet(CK_tensor_table_t* CK_t, unsigned long long key_cmodes){
    unsigned int pos0 = CK_htHashCode(0, key_cmodes, CK_t->shift);

    if(CK_t->CK_node_list[0][pos0].stat == 1 && CK_t->CK_node_list[0][pos0].key == key_cmodes){
        // printf("Get in table 0 with len: %lu\n", CK_t->CK_node_list[0][pos0].val_ptr.len);
        return CK_t->CK_node_list[0][pos0].val_ptr;
    }
    unsigned int pos1 = CK_htHashCode(1, key_cmodes, CK_t->shift);
    if(CK_t->CK_node_list[1][pos1].stat == 1 && CK_t->CK_node_list[1][pos1].key == key_cmodes){
        // printf("Get in table 1 with len: %lu\n", CK_t->CK_node_list[1][pos1].val_ptr.len);
        return CK_t->CK_node_list[1][pos1].val_ptr;
    }
    tensor_value result;
    result.len = 0;
    return result;
...
void CK_tensor_htFree(CK_tensor_table_t* CK_t){
    free(CK_t->CK_node_list);
    free(CK_t);
}

unsigned int tensor_b_ht_size;
tensor_block_table_t* tensor_b_htCreate(const unsigned int size){
    tensor_block_table_t *t = (tensor_block_table_t*)malloc(sizeof(tensor_block_table_t));
    t->size = size;
    tensor_b_ht_size = size;
    t->list = (tensor_block_node_t**)malloc(sizeof(tensor_block_node_t*)*size);
    unsigned int i;
    for(i = 0; i < size; i++)
        t->list[i] = NULL;
    return t;
...
void tensor_b_htUpdate(tensor_block_table_t *t, unsigned long long key_cmode_b, unsigned long long key_fmode_b, tensor_table_t* b_ht){
    unsigned int pos = tensor_b_htHashCode_size(key_cmode_b, t->size);
    tensor_block_node_t *list = t->list[pos];
    tensor_block_node_t *temp = list;
    while(temp){
        if(temp->key == key_cmode_b){
            tensor_b_htAppendBlockEntry(&temp->entry, key_fmode_b, b_ht);
            return;
        }
        temp = temp->next;
    }
...
void tensor_b_htInsert(tensor_block_table_t *t, unsigned long long key_cmode_b, unsigned long long key_fmode_b, tensor_table_t* b_ht){
    tensor_block_node_t *newNode = (tensor_block_node_t*)malloc(sizeof(tensor_block_node_t));
    tensor_block_entry my_entry;
    newNode->key = key_cmode_b;
    tensor_b_htNewBlockEntry(&my_entry, 0, 0);
    tensor_b_htAppendBlockEntry(&my_entry, key_fmode_b, b_ht);  
    newNode->entry = my_entry;
    unsigned int pos = tensor_b_htHashCode_size(key_cmode_b, t->size);
    tensor_block_node_t *list = t->list[pos];
    newNode->next = list;
    t->list[pos] = newNode;
...
tensor_block_entry tensor_b_htGet(tensor_block_table_t* t, unsigned long long key_b){
    unsigned int pos = tensor_b_htHashCode_size(key_b, t->size);
    tensor_block_node_t *list = t->list[pos];
    tensor_block_node_t *temp = list;
...
int tensor_b_htNewBlockEntry(tensor_block_entry *entry, unsigned int len, unsigned int cap){
    ...
    entry->len = len;
    entry->cap = cap;
    entry->key_FM = malloc(cap * sizeof *entry->key_FM);
    entry->b_ht = malloc(cap * sizeof *entry->b_ht);
    memset(entry->key_FM, 0, cap * sizeof *entry->key_FM);
    memset(entry->b_ht, 0, cap * sizeof *entry->b_ht);
    return 0;    
}

int tensor_b_htAppendBlockEntry(tensor_block_entry *entry, unsigned long long key_FM_b, tensor_table_t* b_ht) {
    if(entry->cap <= entry->len) {
#ifndef MEMCHECK_MODE
        sptNnzIndex newcap = entry->cap + entry->cap/2;
#else
        sptNnzIndex newcap = entry->len+1;
#endif
        unsigned long long *new_key_FM = realloc(entry->key_FM, newcap * sizeof *entry->key_FM);
        tensor_table_t **new_b_ht;
        new_b_ht = (tensor_table_t **)realloc(entry->b_ht, newcap * sizeof(tensor_table_t *));
        entry->cap = newcap;
        entry->key_FM = new_key_FM;
        entry->b_ht = new_b_ht;
    }
    entry->key_FM[entry->len] = key_FM_b;
    entry->b_ht[entry->len] = b_ht;
    ++entry->len;
    return 0;
}
