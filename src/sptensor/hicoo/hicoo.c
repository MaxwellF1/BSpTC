    ...
    const sptElementIndex sc_bits)
{
    ...
    hitsr->bfidx = malloc(sizeof(sptNnzIndexVector));
    return 0;
}
...
#if 0
/**
 * Create a new sparse tensor in BCOO format
 * @param btsr    a pointer to an uninitialized sparse tensor
 * @param nmodes number of modes the tensor will have
 * @param ndims  the dimension of each mode the tensor will have
 * @param nnz number of nonzeros the tensor will have
 */
int sptNewSparseTensorBCOO(
...
)
{
    ...
    btsr->nmodes = nmodes;
    btsr->sortorder = malloc(nmodes * sizeof btsr->sortorder[0]);
    for(i = 0; i < nmodes; ++i) {
        btsr->sortorder[i] = i;
    }
    btsr->ndims = malloc(nmodes * sizeof *btsr->ndims);
    spt_CheckOSError(!btsr->ndims, "BSpTns New");
    memcpy(btsr->ndims, ndims, nmodes * sizeof *btsr->ndims);
    btsr->nnz = nnz;

    /* Parameters */
    btsr->sb_bits = sb_bits; // block size by nnz

    /* Balanced structures */

    result = sptNewNnzIndexVector(&btsr->bptr, 0, 0);
    spt_CheckError(result, "BSpTns New", NULL);
    btsr->binds = malloc(nmodes * sizeof *btsr->binds);
    spt_CheckOSError(!btsr->binds, "BSpTns New");
    for(i = 0; i < nmodes; ++i) {
        result = sptNewBlockIndexVector(&btsr->binds[i], 0, 0);
        spt_CheckError(result, "BSpTns New", NULL);
    }

    btsr->einds = malloc(nmodes * sizeof *btsr->einds);
    spt_CheckOSError(!btsr->einds, "BSpTns New");
    for(i = 0; i < nmodes; ++i) {
        result = sptNewElementIndexVector(&btsr->einds[i], 0, 0);
        spt_CheckError(result, "BSpTns New", NULL);
    }
    result = sptNewValueVector(&btsr->values, 0, 0);
    spt_CheckError(result, "BSpTns New", NULL);
