    ...
    int const tk)
{
    ...
    for(sptNnzIndex k=0; k<kptr->len - 1; ++k) {
        ...
        // if(k_end - k_begin > NNZ_THRESHOLD)
        sptSparseTensorSortIndexRowBlock(tsr, 1, k_begin, k_end, 0, tk);
#if PARTI_DEBUG == 3
 ...
 * @return      mode pointers
 */
int sptPreprocessSparseTensorFidx(
    sptNnzIndexVector * kptr,
    sptIndexVector **kschr,
    sptIndex *nkiters,
    sptIndexVector **kschr_balanced,
    ...
    int const tk)
{
    ...
    sptIndex * mode_order = (sptIndex *)malloc(tsr->nmodes * sizeof(sptIndex));
    ...
    *bfidx = realloc(*bfidx, (kptr->len-1)*sizeof(sptNnzIndexVector));
    printf("size of bfidx: %d\n", malloc_usable_size(*bfidx)/sizeof(sptNnzIndexVector));
    /* Sort blocks in each kernel in Morton-order */
    sptNnzIndex k_begin, k_end;
    /* Loop for all kernels, 0-kptr.len for OMP code */
    #pragma omp parallel for num_threads(tk) 
    for(sptNnzIndex k=0; k<kptr->len - 1; ++k) {
        ...
        if(k_end - k_begin > NNZ_THRESHOLD){
            sptSparseTensorSortIndexRowBlock(tsr, 1, k_begin, k_end, 0, tk);
            sptSparseTensorSetIndicesIndex(tsr, mode_order, num_cmodes , k_begin, k_end, &((*bfidx)[k]));
        }
#if PARTI_DEBUG == 3
    printf("Kernel %"PARTI_PRI_NNZ_INDEX ": Sorted by sptSparseTensorSortIndexMorton.\n", k);
    sptAssert(sptDumpSparseTensor(tsr, 0, stdout) == 0);
    ...
    int const tk)
{
    ...
    // sptPreprocessSparseTensor(&hitsr->kptr, hitsr->kschr, hitsr->nkiters, hitsr->kschr_balanced, hitsr->kschr_balanced_pos, hitsr->nkpars, hitsr->kschr_rest, &hitsr->knnzs, tsr, sb_bits, sk_bits, tk);
    sptPreprocessSparseTensorFidx(&hitsr->kptr, hitsr->kschr, hitsr->nkiters, hitsr->kschr_balanced, hitsr->kschr_balanced_pos, hitsr->nkpars, hitsr->kschr_rest, &hitsr->knnzs, &hitsr->bfidx, tsr, num_cmodes, sb_bits, sk_bits, tk);
...
int sptSparseTensorToBlockHash(
    sptSparseTensorHiCOO *hitsr,
    sptNnzIndex *max_nnzb,
    sptSparseTensor *tsr,
    sptIndex num_cmodes, 
 ...
 * @return      mode pointers
 */
int sptPreprocessSparseBTensor(
    sptSparseTensor *tsr, 
    const sptElementIndex sb_bits,
    int const tk)
{
    ...
    int const tk)
{
    ...
    /* Set BCOO parameters. ndims for type conversion, size_t -> sptIndex */
    sptIndex * ndims = malloc(nmodes * sizeof *ndims);
    spt_CheckOSError(!ndims, "HiSpTns Convert");
    for(i = 0; i < nmodes; ++i) {
        ndims[i] = (sptIndex)tsr->ndims[i];
    ...
    result = sptNewSparseTensorBCOO(btsr, (sptIndex)tsr->nmodes, ndims, (sptNnzIndex)tsr->nnz, sb_bits);
    spt_CheckError(result, "BSpTns Convert", NULL);
    ...
    // sptPreprocessSparseBTensor(tsr, sb_bits, tk);
    ...
    spt_CheckError(result, "BSpTns Convert", NULL);
    for(sptIndex m=0; m<nmodes; ++m)
        sptAppendBlockIndexVector(&btsr->binds[m], (sptBlockIndex)block_begin_prior[m]);
    sptAppendNnzIndexVector(&btsr->bptr, 0);
