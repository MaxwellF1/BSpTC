...
long int sptInIndexVectorStart(sptIndexVector * inds, sptNnzIndex nmodes, sptNnzIndex nnz, sptIndexVector * cand_inds, unsigned long long start) {  
    int mark;
    for (sptNnzIndex i = start; i < nnz; ++i) {
        mark = 1;
        for(sptIndex m = 0; m < nmodes; ++m) {
            if(cand_inds->data[m] != inds[m].data[i] ) {
                mark = 0;
