...
/**
 * Sparse tensor type, BCOO format
 */
typedef struct {
    /* Basic information */
    sptIndex            nmodes;      /// # modes
    ...
    sptIndex			*ndims_block;
    sptNnzIndex			*block_nnz;
    sptNnzIndex         num_nnzblock;
    ...
    sptValueVector            values;      /// non-zero values, length nnz
    // sptValueVector * values;      /// non-zero values, length [nnz][\prod ndims_block]
    /* load/schedule balance information*/
    // TODO
} sptSparseTensorBCOO;
...
typedef struct {
    ...
    sptIndex * ndims;      /// size of each mode, length nmodes
    sptNnzIndex nnz;         /// # non-zeros
    sptIndexVector * binds;       /// indices of each element, length [nmodes][nnz]
    sptElementIndex       sb_bits;         /// block size by nnz
    sptNnzIndex         num_blk;
    sptSparseTensor *spt;
} sptSparseTensorBiCOO;
...
typedef struct {
    ...
    /*guofeng add*/
    sptNnzIndexVector         *bfidx;    // set indices for free modes in each block
